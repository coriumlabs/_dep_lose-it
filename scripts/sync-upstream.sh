#!/usr/bin/env bash
# Rebase this fork's local patches on top of the latest phitoduck/lose-it.
# Safe to re-run: does nothing if already up to date, never force-pushes for you.
set -euo pipefail

UPSTREAM_URL="https://github.com/phitoduck/lose-it.git"
...
if git rebase "upstream/$UPSTREAM_BRANCH"; then
  ...
  echo
  echo "Rebase hit conflicts in the patched file(s) -- resolve them, then 'git rebase --continue'."
